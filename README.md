...
```markdown
    ```java
        ...
        // Считывание целого числа x из консоли
            ...
            //Проверка числа x на количество знаков
            if (x/1000==0 || x/10000>0)
              out.print("NO");
            else {
            //Последовательная попарная проверка цифр на повтор в числе x
            //Выделение цифры xi из числа x для последующей проверки
            for(int i=3, xi, a1; i>0; i--) {
                //Определение разряда цифры xi в числе x
                a1=4-i;
                xi=x;
                xi = xi / (int)Math.pow(10,i);
                //Выделение собственно цифры xi из числа x для последующей проверки
                while(xi>9 && a1>1) {
                  xi = xi % 10;
                  a1--;
                }
                //Выделение цифры xu из числа x для последующей проверки
                for(int u=0, xu, a2; u<i; u++) {
                  a2=4;
                  xu=x;
                  xu = xu / (int)Math.pow(10,u);
                  //Выделение собственно цифры xu из числа x для последующей проверки
                  while(xu>9 && a2>u) {
                    xu = xu % 10;
                    a2--;
                  }
                  //Если цифра xi равна цифре xu, проверка цифр на повтор знаков проваливается
                  if (xu == xi)
                    Repeat = true;
                }
            }
            //Проверка цифр на повтор знаков
            if (!Repeat)
                out.print("YES");
            else
                out.print("NO");
